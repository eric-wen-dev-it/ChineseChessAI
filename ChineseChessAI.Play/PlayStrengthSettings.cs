namespace ChineseChessAI.Play
{
    public sealed class PlayStrengthSettings
    {
        ...
        public static PlayStrengthSettings Sanitize(PlayStrengthSettings? settings)
        {
            ...
            settings.TraditionalMoveTimeMs = Math.Clamp(settings.TraditionalMoveTimeMs, 500, 600000);
            ...
            settings.TraditionalBookMode = settings.TraditionalBookMode?.ToLowerInvariant() switch
            {
                "off" => "off",
                "weighted" => "weighted",
                _ => "best"
